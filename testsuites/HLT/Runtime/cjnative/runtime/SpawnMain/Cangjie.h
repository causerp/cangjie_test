...
/* CJThread-related context data used by ThreadSanitizer */
typedef void* RaceStateHandle;

/* Processor-related context data used by ThreadSanitizer */
typedef void* RaceProcHandle;
#endif
 ...
 */
MRT_EXPORT void* InitUIScheduler();
 ...
 * @retval Return 0 if OK. Otherwise, return an error code.
 */
MRT_EXPORT enum RTErrorCode FiniCJRuntime();
 ...
 * @retval Return 0 if OK. Otherwise, return an error code.
 */
MRT_EXPORT enum RTErrorCode SetRuntimeInitFlag();
 ...
 * @retval Return 0 if OK. Otherwise, return an error code.
 */
MRT_EXPORT enum RTErrorCode SetRuntimeFiniFlag();
/*
 ...
 * @attention SemiCJThreadHandle should be released by API DestroyCJThread.
 * @param  func  [IN]  user defined Cangjie task function prototype.
 * @param  args  [IN]  arguments of the Cangjie task function.
 * @param  data  [IN]  pointer to the cjthread specific data.
 * @param  num   [IN]  number of cjthread specific data, num <= CJTHREAD_KEYS_MAX (now defined to 8 in cjthread).
 ...
 * @attention 1. Only can be called in the semi-cjthread's context.
 *            2. SuspendCJThread will return only after main thread called ResumeCJThread.
 ...
 * @attention ResumeCJThread will return only after co called SuspendCJThread
 ...
 * @attention This API should be called after co finished.
 ...
 * @attention 1. This API should be called before co starts or after co finished, to
 *               avoid memory leak.
 *            2. Afther destoryed, co should not be used again.
 * @retval Return 0 if success. Otherwise, return an error code.
 */
