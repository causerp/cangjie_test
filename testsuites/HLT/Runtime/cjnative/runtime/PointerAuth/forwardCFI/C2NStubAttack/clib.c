...
unsigned long tamperTarget = 0;
typedef void (*func_ptr)();
#define PAGE_SIZE 4096
#define PAGE_ALIGN_DOWN(x) (x & ~(PAGE_SIZE - 1))

void __attribute__((noinline)) tamper() {
    unsigned long bad = 0xdeadbeef;
    unsigned long x = 10;
    unsigned long *stack = &x;
    for (int i = 0; i < 50; i++) {
        unsigned long *addr = (unsigned long *)((unsigned long)stack + (i * 8));
        fprintf(stderr, "%d-%lx: %lx ", i, addr, *addr);
        if ((*addr & 0xffffffffffff) == tamperTarget) {
            fprintf(stderr, "tamper it to %lx", bad);
            *addr = bad;
        }
        fprintf(stderr, "\n");
    }
...
void hook() {
    unsigned long funcAddr = getTargetFuncAddr("CJ_AddSingleModelC2NCount");
    unsigned long *got = getGotAddr(funcAddr);
    tamperGot(got, (unsigned long)badFunc);
}

void cfunc() {
    fprintf(stderr, "this is cfunc, addr is %lx\n", &cfunc);
    tamperTarget = (unsigned long)cfunc;
}
