...
配置后，编译 demo 模块时实际使用的间接依赖 bbb 即为new/path/to/bbb 下的 bbb 模块，aaa 中配置的 path/to/bbb 下的 bbb 模块不会被编译。
...
自行将相应的 hello.c 文件编成 .so库（在该文件路径执行 clang  hello.c -o libhello.so）
...
debug/release 模式下对应 target 的配置
debug/release 无关的对应 target 的配置
公共配置项
...
必须直接包含至少一个仓颉代码文件；
其父包（包括父包的父包，直至 root 包）也为有效源码包。其中，模块 root 包不存在父包，因此仅需满足条件 1。
...
进行编译流程前，首先编译 build.cj；
执行 pre-build 对应的功能函数；
进行 cjpm build 编译流程；
编译流程顺利结束后，cjpm 会执行 post-build 对应的功能函数。
构建脚本支持的命令如下：
