...
${output_dir}${case}
cjc --output-type=exe --int-overflow wrapping --jc-options="+pgo -jprofile=${output_dir}/jprofs/${case}.jprof" ${case_path} -o ${output_dir}${case}.out

get_result() {
    cd ${output_dir}
    export JETVMPROP="-Xmx2g"
    res=`./${case}.out`
    echo "${case_dir}:${res}" >> ${curPath}/cj_aot_pgo.log
}
