...
CJOPTS := -ld-options="-L${BENCH_DIR}/julia-benchmark/OpenBLAS -lopenblas"
PIDIGITSOPTS := -I ${BENCH_DIR}/benchmarks-game/ -L ${BENCH_DIR}/benchmarks-game/gmp-6.2.0/lib -lgmp
...
%.gcc_run: %.c
	-$(GCC) -pipe -Wall $(COPTS) $(GCCOPTS) $< -o $@ -lm
...
%.cj_run: %.cj
	-$(CJC) --fast-math -O2 -V --int-overflow wrapping $< -o $@

randmatmul.cj_run: randmatmul.cj
	-$(CJC) -O2 -V --int-overflow wrapping $(CJOPTS) $< -o $@


%.cj_compile_run: %.cj_compile
	-cp $< main.cj
...
pidigits.swift_run: pidigits.swift
	-$(SWIFT) $< -Ounchecked $(PIDIGITSOPTS) -o $@

%.swift_run: %.swift
	-$(SWIFT) $< -Ounchecked  -o $@

%.swift_compile_run: %.swift_compile
	-cp $< main.swift
