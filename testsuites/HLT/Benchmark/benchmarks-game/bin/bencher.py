...
filters = {
    'onlydirs': frozenset(), 'allow': [], 'only': [], 'ignore': []
}
...
def configure(ini):
    global runs, repeatevery, cutoff, delay, maxtime, logfilemax, outputmax, \
        make, makefile, affinitymask, perfrecord
...
def callMake(p):
    ...
    with open(makeName(), 'a') as mf:
        if isexe(makeExeName):
            ...
            if logger:
                logger.debug('%s %s - %s program not found',
...
def cmdTemplate(p):
    ...
    if  perfrecord != '' and os.system('perf') != 0:
        s = perfrecord + ' ' + s
    if logger:
...
def callHighlightSourceCodeMarkup(p):
    ...
    cmd = ['highlight', '--fragment', '--style=edit-eclipse', '--force', '--class-name=NONE', '-i', join(codedir, p.codeName), '-o', join(codedir, p.highlightName)
           ]
...
def writeHeader(f):
    f.write('name,lang,version,arg(n),size(B),cpu(s),mem(KB),status,cpu_load,elapsed(s)\n')
