...
do
    ...
    c_file=`echo $cj_file | sed 's/.cj/.c/g'`
    clang -shared -fPIC $c_file -o lib$module_name.so >>$log_name 2>&1
    if [ $? -ne 0 ]
    then
        echo "ERROR-CJ:BUILD FILE: ${c_file}" >>$log_name
        continue
    fi
    cjc --error-count-limit=all -L . -l $module_name $cj_file $options -o $module_name.out >>$log_name 2>&1
    if [ $? -ne 0 ]
    then
...
rm -rf lib$module_name.so $module_name.out
cd $cwd
exit
