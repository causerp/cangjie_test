...
@Fork(1)
public class BenchmarkRegexCaret {
    ...
    public void setup() {
        ...
        String matchString1 = "zo" + new String(new int [stringSize -2], 0, stringSize - 2).replace('\0', 'n');
        Pattern regex = Pattern.compile(regexString);
        matcher = regex.matcher(matchString1);
    }

    @Benchmark
    public void BenchmarkRegexCaret_MatchCnt() {
        while (matcher.find()) {
    ...
    public static void main(String[] args) throws RunnerException {
        Options opt = new OptionsBuilder()
                .include(BenchmarkRegexCaret.class.getSimpleName())
                .forks(1)
                .build();
        new Runner(opt).run();
    }
