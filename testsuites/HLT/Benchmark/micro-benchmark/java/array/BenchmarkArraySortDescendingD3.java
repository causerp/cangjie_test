...
@Fork(1)
public class BenchmarkArraySortDescendingD3 {
    ...
    @Benchmark
    public void BenchmarkArraySortDescendingD3_N32() {
        for (int[][] arr : arr32) {
            for (int[] ints: arr) {
    ...
    @Benchmark
    public void BenchmarkArraySortDescendingD3_N256() {
        for (int[][] arr : arr256) {
            for (int[] ints: arr) {
                mergeSort(ints, 0, ints.length -1);
            }
    ...
    public static void main(String[] args) throws RunnerException {
        Options opt = new OptionsBuilder()
                .include(BenchmarkArraySortDescendingD3.class.getSimpleName())
                .forks(1)
                .build();
        new Runner(opt).run();
    }
