...
@Fork(1)
public class BenchmarkArrayDeque_addFirst {
    ...
    @Benchmark
    public void BenchmarkArrayDeque_addFirst_Int64(Blackhole blackhole) {
        ArrayDeque_Int64.addFirst(0L);
    }

    @Benchmark
    public void BenchmarkArrayDeque_addFirst_UInt8(Blackhole blackhole) {
        ArrayDeque_UInt8.addFirst(aByte);
    }

    @Benchmark
    public void BenchmarkArrayDeque_addFirst_Float64(Blackhole blackhole) {
        ArrayDeque_Float64.addFirst(3.14d);
    }

    @Benchmark
    public void BenchmarkArrayDeque_addFirst_Int64_G16(Blackhole blackhole) {
        for (long i = 0L; i < 16; i++) {
            ArrayDeque_Int64_N24.addFirst(i);
        }
    }

    @Benchmark
    public void BenchmarkArrayDeque_addFirst_Int64_G128(Blackhole blackhole) {
        for (long i = 0L; i < 128; i++) {
            ArrayDeque_Int64_N192.addFirst(i);
        }
    }

    @Benchmark
    public void BenchmarkArrayDeque_addFirst_Int64_G1024(Blackhole blackhole) {
        for (long i = 0L; i < 1024; i++) {
            ArrayDeque_Int64_N1536.addFirst(i);
        }
    }

    public static void main(String[] args) throws RunnerException {
        Options opt = new OptionsBuilder()
                .include(BenchmarkArrayDeque_addFirst.class.getSimpleName())
                .forks(1)
                .build();
        new Runner(opt).run();
    }
