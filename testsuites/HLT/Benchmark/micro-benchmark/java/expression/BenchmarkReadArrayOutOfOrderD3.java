...
@Fork(1)
public class BenchmarkReadArrayOutOfOrderD3 {
    static int num32 = 32;
    ...
    @Benchmark
    public void BenchmarkReadArrayOutOfOrderD3_N32(Blackhole blackhole) {
        int srcArr = 0;
        for (int k = 0; k < num32; k += 64) {
            for (int j = 0; j < num32; j += 64) {
                for (int i = 0; i < num32; i += 64) {
                    srcArr = targetArr32[k][j][i];
                }
            }
        }
    ...
    @Benchmark
    public void BenchmarkReadArrayOutOfOrderD3_N256(Blackhole blackhole) {
        int srcArr = 0;
        for (int k = 0; k < num256; k += 64) {
            for (int j = 0; j < num256; j += 64) {
                for (int i = 0; i < num256; i += 64) {
                    srcArr = targetArr256[k][j][i];
                }
            }
        }
        blackhole.consume(srcArr);
    ...
    public static void main(String[] args) throws RunnerException {
        Options opt = new OptionsBuilder()
                .include(BenchmarkReadArrayOutOfOrderD3.class.getSimpleName())
                .forks(1)
                .build();
        new Runner(opt).run();
    }
