...
'''
aux_template = '''let f{i} = {{ arg : String => logStr += arg + " |> (f{i}"; {} }}
    let g{i} = G{i}()
    logStr = ""
    (f{i} ~> g{i})("1")
...
var logStr = ""
''' + ''.join([decl % (i, subtypes[i][1], ' logStr += " ~> g{i})" ') for i in range(len(subtypes))]),
'\n\n    '.join([aux_template.format(default_value(subtypes[i][0]), i=i) for i in range(len(subtypes))]))
...
negative_tests = [template % ('''Checks that composition f ~> g is not permitted when return type of f is %s
                and argument type of g is %s.''' % (t1, t0), 'compileonly', 'yes', 'no', '', decl % ('', t1, ' '),
                '''let f = { arg : String => %s }
    let g = G()
    (f ~> g)("1")''' % default_value(t0)) for t0, t1 in non_subtypes]
