...
var logStr = ""
''' + ''.join([decl.format('logStr += arg + " |> (f%s"; %s' % (i, default_value(subtypes[i][0])), subtypes[i][1],
                           'logStr += " ~> g%s)" ' % i, i=i) for i in range(len(subtypes))]),
'\n\n    '.join([aux_template.format(i=i) for i in range(len(subtypes))]))
...
negative_tests = [template % ('''Checks that composition f ~> g is not permitted when return type of f is %s
    ...
    decl.format(default_value(t0), t1, '', i=''), '''let f = F()
    let g = G()
    (f ~> g)("1")''') for t0, t1 in non_subtypes]
