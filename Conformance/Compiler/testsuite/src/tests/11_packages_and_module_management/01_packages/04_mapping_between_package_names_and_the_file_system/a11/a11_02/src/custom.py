...
output_main = join(output_dir, 'test_a11_02{}'.format(bin_ext))
pkg_dir_a = join(source_dir, "A")
pkg_out_a = join(output_dir, 'build', 'default', 'libA.a')
...
pkg_out_b = join(output_dir, 'build', 'default', 'libAB.a')
...
on_compile(cjc(option='--import-path {} '.format(join(output_dir, "build")) + 
               ...
               ' -lA -lAB', 
               source='{}'.format(join(source_dir, "test_a11_02.cj")), output=output_main))
