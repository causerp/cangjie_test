...
on_compile(cjc(option=f'--import-path "{output_dir}" {depends} {macro_option} ', source=join(source_dir, 'aux_macros_02.cj'),
               output=macro_output))


on_compile(f'{self.cfg.cjc} {self.cfg.cjc_flags} --import-path "{output_dir}" "{join(source_dir, "test_a04_01.cj")}" '
           f'{depends} '
