...
def gen_test(ty):
    global n
    testname = "test_a01_" + format(n, "02d")
    testfile = os.path.join(output_dir, testname + ".cj")
    global template_cj
    with open(testfile, 'w') as f:
        content = template_cj.render(dict(Type=ty, Val=ctypes[ty][2], num=format(n, "02d")))
        f.write(content)
...
def gen_test_ptr(ty):
    ...
    global template_cj
    with open(testfile, 'w') as f:
        content = template2_cj.render(dict(Type=ty, Val=ctypes[ty][2], num=format(n, "02d")))
        f.write(content)
