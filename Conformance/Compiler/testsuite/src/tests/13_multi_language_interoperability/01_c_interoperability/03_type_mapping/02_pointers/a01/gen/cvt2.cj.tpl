...
/*
  ...
  @Description:  Cangjie type 'CPointer<CPointer<{{CJType}}>>' is mapped to C type '{{CType}} **'.
...
foreign func func_01(): CPointer<CPointer<{{CJType}}>>
foreign func func_02(x: CPointer<{{CJType}}>): Bool

main() {
    var PtrPtr : CPointer<CPointer<{{CJType}}>> = unsafe{ func_01() }
    Assert.isFalse(PtrPtr.isNull())
    var Ptr : CPointer<{{CJType}}> = unsafe{ PtrPtr.read() }
