...
def gen_cvt_ret(ty):
    global n
    testfile = "test_a09_" + format(n, "02d") + ".cj"
    testfile = os.path.join(output_dir, testfile)
    global template_cj
    with open(testfile, 'w') as f:
        content = template_cj.render(dict(CJType=ty, CJVal=ctypes[ty][2], num=format(n, "02d")))
        f.write(content)
...
def gen_cvt_arg(ty):
    ...
    global template_cj
    with open(testfile, 'w') as f:
        content = template2_cj.render(dict(CJType=ty, CJVal=ctypes[ty][2], num=format(n, "02d")))
        f.write(content)
